...
exports.busted = function(fileContents, options) {
  ...
  options = {
   ...
   var protocolRegEx = /^http(s)?/, elements = $('script[src], link[rel=stylesheet][href]'), relativeRegEx = /^\.\./;
  ...
  for (var i = 0, len = elements.length; i < len; i++) {
    ...
    if (!protocolRegEx.test(originalAttrValue)) {
        if(relativeRegEx.test(originalAttrValue)) {
            originalAttrValue = originalAttrValue.replace(/^\.\.\//, '');
        }
